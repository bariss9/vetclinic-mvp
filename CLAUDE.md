...
- **appointments** — CRUD, status enum (PENDING/SCHEDULED/COMPLETED/CANCELLED/NO_SHOW/UNCERTAIN — migration: `add_appointment_status_values`); GET returns full patient + owner info; **JWT guard + ownership filtering**; **rol bazlı status geçiş kısıtları** (bkz. Randevu Durum Yönetimi); `reminderSent Boolean @default(false)` alanı (migration: `add_appointment_reminder`); günlük cron (09:00) yarınki SCHEDULED randevular için owner'a hatırlatma maili; günlük cron (08:00) tarihi 2+ gün geçmiş SCHEDULED randevuları UNCERTAIN'e çeker; **slot sistemi** (2026-07-09): `GET /appointments/available-slots?clinicName=X&date=YYYY-MM-DD` (JwtAuthGuard + ThrottlerGuard; `:id` rotasından ÖNCE tanımlı kalmalı) — 09:00-17:00 arası saat başı 9 slot, aynı clinicName + gün içinde PENDING/SCHEDULED randevunun düştüğü saat dilimleri elenir, geçmiş saatler elenir, `{ clinicName, date, slots: [{ time, iso }] }` döner; `create()` çakışma koruması: clinicName doluysa aynı saat dilimi (saat başı bucket) için aktif randevu varsa 409 "Bu saat dolu" (`assertSlotFree`); slot saatleri sunucu lokal zamanında kurulur (MVP: sunucu + istemci aynı makine)
...
4. **Saat Seç** (2026-07-09'da yeniden yazıldı) — tarih gezgini (← başlık →; ← bugünden geriye gitmez) + `GET /appointments/available-slots?clinicName=X&date=YYYY-MM-DD`'den gelen müsait saatler (09:00-17:00, saat başı); tarih değişince yeniden fetch (bayat cevaplara karşı `slotReqSeq` ref guard), loading/hata/boş gün ("Bu gün müsait randevu yok") durumları; eski hardcoded `generateSlots()` + `GET /appointments` istemci filtresi KALDIRILDI; tarih başlığı ve YMD üretimi timezone-güvenli (`toYMD`/`formatDateTR` — lokal parçalardan, `toISOString` yok)
...
### Klinik Bildirimi + PENDING Hatırlatma (2026-07-09)
- Migration: `add_appointment_reminder_count` — Appointment'a `reminderCount Int @default(0)` eklendi
- **Anlık bildirim**: `appointments.service.ts` → `create()` sonrası `void notifyClinicOfNewAppointment(appt)` (fire-and-forget — mail randevu cevabını bekletmez, hata randevuyu geri almaz, sadece WARN); klinik e-postası `resolveClinicEmail(clinicName)` ile: `Clinic.name === clinicName` eşleşen kaydın `user.email`'i, bulunamazsa null → WARN + atla (Overpass kliniklerinin çoğunun hesabı yok — beklenen durum); mail: hasta sahibi, hayvan, tarih/saat, klinik + `http://localhost:5173` login linki; `AppointmentsService` artık `MailService` inject ediyor (@Global MailModule)
- **PENDING hatırlatma cron'u**: `reminders.service.ts` → `@Cron('*/30 * * * *')` `remindPendingAppointments()`: status=PENDING, `reminderCount < 3`, tarihi geçmemiş randevular; kliniğe "hâlâ onay bekliyor" maili (hatırlatma N/3 bilgisiyle); **sayaç her denemede artar** (mail başarısız/klinik hesabı yok dahil) — Resend test modunda doğrulanmamış adreslere gönderim kalıcı başarısız olduğundan, sadece-başarıda-artır süresiz retry döngüsü yaratırdı; per-item try/catch (`runReminders` pattern'i); `RemindersService` `resolveClinicEmail` için `AppointmentsService` inject ediyor
